...
__device__ void load_tile(const float* K, const float* V, half k_buf[BC][128], float v_buf[BC][128], int n, int N, int d, int idx) {
    for (int i = 0; i < BC; i++) {
        for (int c = idx; c < 128; c += blockDim.x * blockDim.y) {
            float kv = (n + i < N && c < d) ? K[(n+i) * d + c] : 0.0f;
            v_buf[i][c] = (n + i < N && c < d) ? V[(n+i) * d + c] : 0.0f;
            k_buf[i][c] = __float2half(kv); 
        }
    }
}
...
void __global__ flash(const float* Q, const float* K, const float* V, float* output, int M, int N, int d) {
    ...
    __shared__ half q_half[WARPS][16][128], k_half[2][BC][128];
    __shared__ float v[2][BC][128];
    ...
    load_tile(K, V, k_half[0], v[0], 0, N, d, idx);
    __syncthreads();
    ...
    for (int n = 0; n < N; n += BC ) {
        ...
        if (n + BC < N) {
            load_tile(K, V, k_half[next], v[next], n + BC, N, d, idx);
        }
        ...
        for (int dc = 0; dc < d; dc += 16) {
            ...
            wmma::load_matrix_sync(b_frag, &k_half[cur][0][dc], 128);
        ...
        for (int r = 0; r < ROWS_PER_WARP; r++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                for (int i = 0; i < BC; i++) {
                    acc += p[warp][r][i] * v[cur][i][j + tid * 4];
                }
                out[r][j] = acc;
            }
        }
...
extern "C" void solve(const float* Q, const float* K, const float* V, float* output, int M, int N, int d) {
    ...
    cudaFuncSetAttribute(flash, cudaFuncAttributePreferredSharedMemoryCarveout, cudaSharedmemCarveoutMaxShared);
